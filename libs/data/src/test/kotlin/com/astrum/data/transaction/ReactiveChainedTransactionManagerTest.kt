...
class ReactiveChainedTransactionManagerTest : CoroutineTestHelper() {
    ...
    fun getReactiveTransaction() = blocking {
        ...
        every { reactiveTransactionManager.getReactiveTransaction(transactionTemplate) } returns Mono.just(
            genericReactiveTransaction
        )

        reactiveChainedTransactionManager.registerTransactionManager(reactiveTransactionManager)

        val reactiveTransaction =
            reactiveChainedTransactionManager.getReactiveTransaction(transactionTemplate)
                .awaitSingle()
