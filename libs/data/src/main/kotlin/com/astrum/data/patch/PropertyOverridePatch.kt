...
class PropertyOverridePatch<S : Any, T : Any>(
    ...
    init {
        target.memberProperties.forEach { targetProperty ->
            ...
            val sourceProperty = source.memberProperties.find { it.name == targetProperty.name }
                ?: throw IllegalArgumentException("${target.simpleName}.${targetProperty.name} is not exists.")
