...
suspend fun <ID : Any, T : Any> Storage<ID, T>.createIndexes(
    clazz: KClass<T>,
    indexName: (KProperty1<T, *>) -> String = { it.name }
) {
    ...
    indexes.forEach { (_, properties) ->
        val sortedProperties =
            properties.map { it to columnName(it) }.sortedBy { (_, columnName) -> columnName }
        createIndex(
...
suspend fun <ID : Any, T : Any> Storage<ID, T>.getIndexNameAndValue(
    keys: List<String>,
    values: List<Any?>
): Pair<String, Any>? {
