package com.astrum.data.transaction
...
class ReactiveChainedTransactionManager : ReactiveTransactionManager {
    ...
    private val transactionManagers =
        Collections.synchronizedList(mutableListOf<ReactiveTransactionManager>())
            ...
            if (definition == null) {
                ...
                try {
                    for (transactionManager in transactionManagers) {
                        chainedTransaction.registerTransactionManager(
                            definition,
                            transactionManager
                        )
                    }
                    ...
                    for (transactionManager in transactionManagers) {
                        try {
                            ...
                            logger.warn(
                                "Rollback exception (" + transactionManager + ") " + ex2.message,
                                ex2
                            )
                        }
                    }
    ...
    override fun commit(transaction: ReactiveTransaction): Mono<Void> {
        return mono {
            ...
            for (transactionManager in transactionManagers.reversed()) {
                if (commit) {
                    ...
                    try {
                        ...
                        logger.warn(
                            "Rollback exception (after commit) (" + transactionManager + ") " + ex.message,
                            ex
                        )
                    }
                }
            ...
            if (commitException != null) {
                val firstTransactionManagerFailed =
                    commitExceptionTransactionManager === getLastTransactionManager()
        ...
        return mono {
            ...
            for (transactionManager in transactionManagers.reversed()) {
                try {
                    ...
                    if (rollbackException == null) {
                        ...
                        logger.warn(
                            "Rollback exception (" + transactionManager + ") " + ex.message,
                            ex
                        )
                    }
                }
            ...
            if (rollbackException != null) {
                throw UnexpectedRollbackException(
                    "Rollback exception, originated at (" + rollbackExceptionTransactionManager + ") " + rollbackException.message,
                    rollbackException
                )
            }
        }.mapNotNull { null }
