...
import com.astrum.data.repository.update
...
class MigrationManager(
    ...
    private val migrationCheckpointRepository = R2DBCRepositoryBuilder<MigrationCheckpoint, ULID>(
        entityOperations,
    ...
    private suspend fun waitingComplete(version: Int) {
        while (true) {
            val exits = migrationCheckpointRepository.findOne(
                where(MigrationCheckpoint::version).`is`(version)
            )
            when (exits?.status) {
